...
# Convert FASTQ files to DCC (digital count conversion) files

import "../../wf-common/wdl/structs.wdl"

workflow fastq_to_dcc {
	input {
		String team_id
	...
	String sub_workflow_name = "fastq_to_dcc"
	String convert_fastq_to_dcc_task_version = "1.0.0"

	Array[Array[String]] workflow_info = [[run_timestamp, workflow_name, workflow_version, workflow_release]]

	String workflow_raw_data_path_prefix = "~{raw_data_path_prefix}/~{sub_workflow_name}"
	String dcc_raw_data_path = "~{workflow_raw_data_path_prefix}/convert_fastq_to_dcc/~{convert_fastq_to_dcc_task_version}"

	scatter (sample_object in samples) {
		String convert_fastq_to_dcc_output = "~{dcc_raw_data_path}/~{sample_object.sample_id}.geomxngs_out_dir.tar.gz"
	}

	# For each sample, outputs an array of true/false: [convert_fastq_to_dcc_complete]
	call check_output_files_exist {
		input:
			convert_fastq_to_dcc_output_files = convert_fastq_to_dcc_output,
	...
	scatter (sample_index in range(length(samples))) {
		...
		String convert_fastq_to_dcc_complete = check_output_files_exist.sample_preprocessing_complete[sample_index][0]
		...
		if (convert_fastq_to_dcc_complete == "false") {
			call convert_fastq_to_dcc {
				input:
		...
		File geomxngs_dcc_zip_output = select_first([convert_fastq_to_dcc.geomxngs_dcc_zip, fastq_to_dcc_geomxngs_dcc_zip]) #!FileCoercion
		File geomxngs_output_tar_gz_output = select_first([convert_fastq_to_dcc.geomxngs_output_tar_gz, fastq_to_dcc_geomxngs_output_tar_gz]) #!FileCoercion
	}
...
task check_output_files_exist {
	input {
		Array[String] convert_fastq_to_dcc_output_files
	...
	command <<<
		...
		done < <(paste ~{write_lines(convert_fastq_to_dcc_output_files)})
	>>>
